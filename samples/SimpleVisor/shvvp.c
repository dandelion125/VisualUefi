    ...
    )
{
	...
	{
		ShvOsDebugPrint("SHV is already running!\n");
		status = SHV_STATUS_ALREADY_EXISTS;
		goto Failure;
	}
    ...
    if (!ShvVmxProbe())
    {
		ShvOsDebugPrint("ShvVmxProbe failed\n");
    ...
    if (vpData == NULL)
    {
		ShvOsDebugPrint("ShvVpAllocateData failed\n");
    ...
    if (status != SHV_STATUS_SUCCESS)
    {
        ...
        //
		ShvOsDebugPrint("ShvVpInitialize failed\n");
        goto Failure;
    }
    ...
    if (!ShvIsOurHypervisorPresent())
    {
        ...
        //
		ShvOsDebugPrint("ShvIsOurHypervisorPresent failed\n");
