    ...
    )
{
    ...
    //
	ShvVmxAdjustCr0((UINT32 *)&Registers->Cr0);
	ShvVmxAdjustCr4((UINT32 *)&Registers->Cr4);
    ...
    )
{
	...
	//
	// Make sure that any change to CR0 and CR4 will cause a VM-Exit
	// so we can adjust the value
	//
	__vmx_vmwrite(CR0_GUEST_HOST_MASK, state->Cr0);
	__vmx_vmwrite(CR4_GUEST_HOST_MASK, state->Cr4);
