...
// See UefiRuntimeLib!RuntimeLibExitBootServicesEvent
extern BOOLEAN mEfiAtRuntime;

// We assume that if mEfiAtRuntime is TRUE we're in the OS and we can use "INT 3"
#define RUNTIME_DEBUGBREAK() \
	{ \
		if (mEfiAtRuntime) \
		{ \
			__debugbreak(); \
...
#define SHV_STATUS_ALREADY_EXISTS	-4
